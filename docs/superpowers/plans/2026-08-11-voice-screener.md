...
- [ ] **Step 4: Ответить на шесть вопросов по собранному JSON**
...
5. **Приходит ли `conversation.item.input_audio_transcription.segment`?** В схеме API это событие есть, и оно несёт `start`, `end` и метку говорящего — то есть пословные тайминги прямо в потоке. Документация сама себе противоречит: одна страница разрешает нужную модель, другая говорит «в realtime не поддерживается». Тридцать минут проверки закрывают вопрос, который иначе останется гаданием. Если событие приходит — цитаты можно поднять с уровня фразы до уровня слова без единого лишнего вызова.
6. **Насколько серверные тайминги расходятся с записью?** Замерить: взять последнюю реплику кандидата в десятиминутном разговоре, вычислить её позицию в файле через `audio_offset_sec` и послушать. `audio_start_ms` считается от аудио, записанного в буфер **на стороне сервера**, а по WebRTC этот счётчик идёт от декодированного потока и может разъезжаться с локальной записью при потерях пакетов. Если к концу разговора расхождение больше секунды — цитаты в конце интервью будут играть не те слова, и тогда нужен пункт из задачи 21.
...
### Task 16: Запуск анализа и подготовка записи

`runAnalysis` — единственное место, где анализ запускается: его зовёт `/api/turns` при
завершении разговора и роут `/api/analyze` при ручном повторе с карточки. `prepareAudio`
делает из чанков перематываемый файл — его играет карточка.

**Files:**
- Modify: `src/lib/analyze/run.ts` (заглушка из задачи 8)
- Create: `src/app/api/analyze/route.ts`
- Create: `src/lib/audio/prepare.ts`
- Create: `src/app/api/audio/prepare/route.ts`
- Modify: `src/app/api/turns/route.ts` (вызов `prepareAudio`)
...
- Produces:
  ...
  - `prepareAudio(sessionId: string): Promise<{ audioUrl: string | null }>` — идемпотентна
  - `POST /api/analyze` с телом `{ sessionId }` → `{ ok: true, droppedClaims }`
  - `POST /api/audio/prepare` с телом `{ sessionId }` → `{ audioUrl }`
...
### Task 21 (необязательная): пословные тайминги через разбор записи

**Делать только если базовая система работает и до сдачи остаётся время.** Это улучшение,
а не требование: без него карточка работает, цитаты играют пофразно.

Проблема, которую это решает, двойная. Во-первых, серверные тайминги считаются от потока,
принятого сервером, а запись ведётся локально — по WebRTC эти две шкалы могут разъезжаться.
Во-вторых, у нас нет пословной точности и нет уверенности модели в распознавании отдельных слов.

Решение — один пакетный вызов распознавания **по нашей же записи** после разговора.
Тайминги, полученные так, привязаны к файлу **по построению**: расхождение невозможно,
потому что распознаётся ровно тот файл, который играет карточка.

**Files:**
- Create: `src/lib/audio/align.ts`
- Modify: `src/lib/analyze/index.ts` (сопоставление цитат со словами)

**Что делать:**

1. После `prepareAudio` отправить готовый файл на пакетное распознавание с запросом
   пословных таймингов и уверенности. Выбирать вендора **по данным на спонтанной речи
   неносителей языка, а не по общему рейтингу**: в независимом сравнении порядок вендоров
   меняется на противоположный между читаемой и спонтанной речью, а наше интервью —
   спонтанное. `gpt-4o-transcribe` уже есть в проекте и не требует нового вендора,
   но проверить его на реальных записях обязательно.
   Отдельно про точность самих границ слова: у выделенных инструментов выравнивания
   средняя ошибка границы порядка 10–18 мс, у распознавателей общего назначения —
   около 110 мс. Если пословная подсветка окажется важна, разница на порядок.
2. Сохранить `words[]` c `{word, startSec, endSec, confidence}` рядом с транскриптом.
3. В `validateEvidence` дополнительно находить границы цитаты по этому списку слов —
   тогда фрагмент играется точно по фразе, а не по всей реплике с подушкой.
4. **Отбрасывать утверждения, опирающиеся на слова с низкой уверенностью распознавания.**
   Это прямое усиление главного принципа: цитата, в которой распознаватель сам не уверен,
   не может подкреплять вывод о кандидате.

Стоимость — порядка четырёх центов на интервью. Задержки не добавляет: шаг асинхронный
и повторяемый, упасть он может только в свою собственную ошибку, не задев разговор.

---

## Self-Review

**Покрытие спека.** Каждый раздел спека закрыт задачей: §3 архитектура — задачи 1, 7, 10;
§3.1 контракт Realtime — задачи 2, 7, 10; §4 флоу кандидата — задачи 5, 11; §5 данные
разговора — задачи 3, 6, 8; §5 стоимость — задача 20 шаг 2; §6 запись аудио и ремукс —
задачи 9, 16; §7 анализ — задачи 12, 13, 14, 15, 16; §7 порог достаточности — задачи 12, 15,
17; §7.1 карточка и дашборд — задачи 17, 18; §8 сбои — задачи 11, 16, 19; §9 границы —
задача 14 (промпты) и 17 (дисклеймер); §10 стек — задача 1; §11 модель данных — задача 4;
§12 проверка — тесты в задачах 5–16 и прогоны в 19; §13 график — порядок задач;
§14 артефакты — задача 20; §15 слабые места — задача 20 шаг 3.

**Заглушек нет.** Каждый шаг с кодом содержит код целиком; ни одного «аналогично задаче N»,
ни одного «добавить обработку ошибок» без указания какой именно. Единственная намеренная
заглушка — `runAnalysis` в задаче 8, и она наполняется в задаче 16; это названо в обоих местах.

**Согласованность типов.** `Turn`, `Metrics`, `Card`, `Evidence` определены в задаче 3 и
используются дальше без переименований. `assembleTurns` и `computeAudioOffset` (задача 6) →
`/api/turns` (задача 8) → `buildCard` (задача 15) → карточка (задача 17) работают на одном
и том же `Turn`. `validateEvidence`/`keepSupported` (задача 13) вызываются в задаче 15
с теми же сигнатурами. `hasEnoughSpeech` (задача 12) определяет, вернёт ли `buildCard`
блок оценки или `Insufficient`, и карточка разбирает это через `isInsufficient` (задача 3).
`questionLabel` в `QuestionCoverage` заполняется в задаче 15 из `role.questions[].label`,
а не приходит от модели — поэтому его нет в `StructureResult` задачи 14, и это осознанно.

**Три места, где план обязан уступить реальности.** Точное значение `turn_detection`
(задача 7) берётся из результатов задачи 2. Значение `OPENAI_ANALYSIS_MODEL` (задача 14) —
из живого списка моделей. Качество промптов (задача 15 шаг 6) проверяется глазами на
четырёх фикстурах, а не тестом: иначе его не проверить. План не угадывает эти значения,
а требует их измерить.

**Что проверено по первоисточникам, а не по памяти.** Контракт Realtime API и наличие
серверных таймингов; поддерживаемое подмножество JSON Schema под `strict` и форма запроса
Responses API; лимиты Vercel по времени и размеру тела; поведение `MediaRecorder` и
браузеров при перемотке; пригодность библиотек ремукса. Выводы, изменившие план, отмечены
в тексте задач комментариями рядом с кодом, чтобы исполнитель видел причину, а не только
результат.

**Риск, который план не устраняет полностью.** Точность привязки цитат к записи держится
на одной величине — `audio_offset_sec`. Она вычисляется из первого события начала речи
и проверяется тестами (задача 6) и глазами на живой записи (задача 17 шаг 6). Если она
окажется неверной, цитаты будут играть не те слова — поэтому проверка вынесена в отдельный
пункт приёмки, а не растворена в «проверить карточку».
